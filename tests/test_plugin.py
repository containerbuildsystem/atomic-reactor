...
def test_load_plugins(docker_tasker, workflow, runner_type, tmpdir):
    """
    test loading plugins
    """
    runner = runner_type(docker_tasker, mock_workflow(tmpdir, workflow), None)
...
def test_prebuild_plugin_failure(docker_tasker, workflow):  # noqa
    setattr(workflow, 'builder', X())
...
def test_required_plugin_failure(tmpdir, docker_tasker, workflow, runner_type, required):
    """
...
def test_verify_required_plugins_before_first_run(caplog, tmpdir, docker_tasker, workflow,
...
def test_check_no_reload(caplog, tmpdir, docker_tasker, workflow):
    """
...
def test_buildstep_phase_build_plugin(caplog, tmpdir, docker_tasker, workflow,
                                      success1, success2):
    """
...
@pytest.mark.parametrize('success1', [True, False])  # noqa
def test_buildstep_phase_build_plugin_failing_exception(tmpdir, caplog, docker_tasker,
                                                        workflow, success1):
    """
    plugin runner should stop after first successful plugin
...
def test_non_buildstep_phase_raises_InappropriateBuildStepError(caplog, tmpdir,
                                                                docker_tasker, workflow):  # noqa
    """
...
def test_no_appropriate_buildstep_build_plugin(caplog, tmpdir, docker_tasker, workflow):  # noqa
    """
