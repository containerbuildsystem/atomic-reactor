...
def test_rpmqa_plugin(caplog, docker_tasker, workflow, base_from_scratch, remove_container_error,
...
def test_rpmqa_plugin_skip(docker_tasker, workflow):  # noqa
    """
...
def test_rpmqa_plugin_exception(docker_tasker, workflow):  # noqa
    mock_docker()
    workflow.source = StubSource()
    workflow.builder = StubInsideBuilder().for_workflow(workflow)
...
def test_dangling_volumes_removed(docker_tasker, workflow, caplog):
...
def test_empty_logs_retry(docker_tasker, workflow):  # noqa
    mock_docker()
    workflow.source = StubSource()
    workflow.builder = StubInsideBuilder().for_workflow(workflow)
    workflow.builder.set_base_from_scratch(False)
...
def test_empty_logs_failure(docker_tasker, workflow):  # noqa
    mock_docker()
    workflow.source = StubSource()
    workflow.builder = StubInsideBuilder().for_workflow(workflow)
    workflow.builder.set_base_from_scratch(False)
