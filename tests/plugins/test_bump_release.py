...
class DFWithRelease(object):
    ...
    def __enter__(self):
        repo = init_repository(self.path)
        ...
        index = repo.index
        index.add(filename)
        author = Signature('Test', 'test@example.com')
        committer = Signature('Test', 'test@example.com')
...
def test_bump_release(tmpdir, label, expected):
    with DFWithRelease(label=label) as (df_path, commit):
        ...
        repo = init_repository(os.path.dirname(df_path))
        ...
        assert ('commit_message' not in args or
