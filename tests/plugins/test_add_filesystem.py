...
def mock_workflow(workflow, build_dir: Path, dockerfile=DEFAULT_DOCKERFILE,
    ...
    if not platforms:
        platforms = ['x86_64']
    workflow.data.plugins_results[PLUGIN_CHECK_AND_SET_PLATFORMS_KEY] = set(platforms)
...
def test_image_build_defaults(workflow, build_dir, resolve_compose, yum_repos):
    ...
    plugin.workflow.data.plugins_results[PLUGIN_RESOLVE_COMPOSES_KEY] = resolve_compose
